...
def set_page_footer():
    ...
    st.markdown(
        'The Real Estate Recommendation Dashboard is made by 3422_ Sanket Veerkar & 3419_Jagdish Rathore')
...
def set_report_sidebar(data):
    ...
    # filters
    invest_opt = st.sidebar.multiselect(
        'Investment Option', data.status.unique(), default=['to buy'])
    house_id = st.sidebar.multiselect('House Id', np.sort(data.id.unique()))
    f_zipcode = st.sidebar.multiselect(
        'Enter zipcode', np.sort(data['zipcode'].unique()))
    f_attributes = st.sidebar.multiselect('Enter columns', data.columns, default=[
                                          'id', 'zipcode', 'price', 'selling_price', 'profit_est', 'best_season_selling_price', 'best_season_profit_est', 'status'])
    f_living_size = st.sidebar.multiselect(
        'Living Size', data.living_size.unique())
...
def set_macro_sidebar(data):
    ...
    f_zipcode = st.sidebar.multiselect(
        'Enter zipcode', np.sort(data['zipcode'].unique()))
    f_order_column = st.sidebar.selectbox(
        'Order Data Overview by column:', data.columns)
...
def dashboard_choice(status):
    ...
    f_dashboard = st.selectbox(
        'Choose Dashboard', ['', 'Macro Dashboard', 'Report Dashboard'])
...
def macro_dashboard(path, url, option):
    ...
    data_analysis['house_total_m2'] = data_analysis['m2_living'] + \
        data_analysis['m2_lot']

    # create price/m²
    data_analysis['price_m2'] = data_analysis['price'] / \
        data_analysis['house_total_m2']
    ...
    physical_attr_dist(
        data_analysis, filters['bedrooms'], filters['bathrooms'], filters['floors'], filters['waterfront'])
...
def data_size_choice():
    ...
    with st.container():
        ...
        option = st.selectbox('Select size', ('', '25% of data',
                              '50% of data', '75% of data', '100% of data'), key='data_size')
        ...
        if option == '':
            ...
            st.warning(
                'Please, note that choosing this option may slow down the report loading.')
        else:
    ...
    for feature in filters:
        if (filters[feature] != []) & (feature != 'attributes') & (report_element == 'table'):
            final_mask = np.logical_and(
                final_mask, data[feature].isin(filters[feature]))
...
def filter_data(data, filters, report_element, dashboard=''):
    ...
    filters_list = {key: filters[key]
                    for key in filters if isinstance(filters[key], list)}
    ...
    if dashboard == 'macro':
        ...
        if ((house_id != []) & (f_attributes != [])) & (report_element == 'table'):
            data_filtered = data_filtered.loc[mask & data['id'].isin(
                house_id), f_attributes]
        ...
        if 'bedrooms' in data_filtered.columns:
            data_filtered = data_filtered.loc[data_filtered.bedrooms <
                                              filters['bedrooms']]
        if 'bathrooms' in data_filtered.columns:
            data_filtered = data_filtered.loc[data_filtered.bathrooms <
                                              filters['bathrooms']]
        if 'floors' in data_filtered.columns:
            data_filtered = data_filtered.loc[data_filtered.floors <
                                              filters['floors']]
        if 'waterfront' in data_filtered.columns:
            data_filtered = data_filtered.loc[data_filtered.waterfront ==
                                              filters['waterfront']]
        if 'yr_built' in data_filtered.columns:
            data_filtered = data_filtered.loc[data_filtered.yr_built <
                                              filters['yr_built']]
        if 'price' in data_filtered.columns:
            data_filtered = data_filtered.loc[data_filtered.price <
                                              filters['price']]
        if 'date' in data_filtered.columns:
            data_filtered = data_filtered.loc[data_filtered.date <
                                              filters['date']]
        ...
        if ((house_id != []) & (f_attributes != [])) & (report_element == 'table'):
            data_filtered = data.loc[mask &
                                     data['id'].isin(house_id), f_attributes]
        elif ((house_id == []) & (f_attributes != [])) & (report_element == 'table'):
...
def report_overview(data):
    ...
    exp_overall = st.expander(
        "Click here to expand/close overall information section.", expanded=True)
    with exp_overall:
        ...
        with c1:
            ...
            st.metric(label='Maximum Expected Profit', value=numerize(
                profit), delta=numerize(gross_revenue) + "%")
        ...
        with c2:
            ...
            st.write(
                '**3. To rise the housing selling price, the company should do renewal works. So what would be good renewal changes?**')
            st.write('')
            st.subheader(
                'Total properties on selected dataset: {:,}'.format(data.shape[0]))
            st.subheader('Total properties suggested to be purchased: {:,}'.format(
                data.loc[data.status == 'to buy'].shape[0]))
...
def statistics_view(data):
    ...
    df3 = data[['m2_living', 'zipcode']].groupby(
    ...
    df_stats = pd.concat([max_, min_, media, median, std],
                         axis=1).reset_index()
...
def top_houses_per_profit(data, st_col):
    ...
    columns = ['id', 'zipcode', 'price', 'selling_price', 'profit_est',
               'best_season_selling_price', 'best_season_profit_est']
    with st_col:

        st.subheader('Top 10 Houses')
        st.write('With selling price suggestion increased based on:')
        st.write(
            'Invested value above region median price value, house condition above 3 and best season to sell')
        st.write(data.loc[data.status == 'to buy', columns].sort_values(
            by='best_season_profit_est', ascending=False).reset_index(drop=True).head(10))
...
def renovation_histograms(house_df: pd.DataFrame):
    ...
    zipcode = st.sidebar.selectbox(
        'Enter the zipcode to check renovation proposals:', unique_zipcodes)
    ...
    if zipcode in unique_zipcodes:
        ...
        houses_to_buy_grouped = houses_to_buy_df.groupby(
            by=['living_size', 'lot_size']).median().reset_index()
        houses_to_compare_grouped = houses_to_compare_df.groupby(
            by=['living_size', 'lot_size']).median().reset_index()

        houses_to_buy_grouped.rename(
            columns={'bedrooms': 'houses_to_buy_bedrooms'}, inplace=True)
        houses_to_buy_grouped.rename(
            columns={'bathrooms': 'houses_to_buy_bathrooms'}, inplace=True)
        houses_to_compare_grouped.rename(
            columns={'bedrooms': 'houses_to_compare_bedrooms'}, inplace=True)
        houses_to_compare_grouped.rename(
            columns={'bathrooms': 'houses_to_compare_bathrooms'}, inplace=True)

        fig, ax = plt.subplots()
        houses_to_compare_grouped['property_size'] = houses_to_compare_grouped['living_size'] + \
            '_'+houses_to_compare_grouped['lot_size']
        houses_to_compare_grouped.plot.bar(x='property_size', y=[
                                           'houses_to_compare_bedrooms', 'houses_to_compare_bathrooms'], ax=ax)
        houses_to_buy_grouped['property_size'] = houses_to_buy_grouped['living_size'] + \
            '_'+houses_to_buy_grouped['lot_size']
        houses_to_buy_grouped.plot.bar(x='property_size', y=[
                                       'houses_to_buy_bedrooms', 'houses_to_buy_bathrooms'], ax=ax, color=['lightblue', 'orange'])
...
def pre_processing(df):
    ...
    df[['m2_living', 'm2_above', 'm2_basement', 'm2_lot']] = df[[
        'sqft_living', 'sqft_above', 'sqft_basement', 'sqft_lot']] * 0.0929

    df = df.drop(['sqft_living', 'sqft_above',
                 'sqft_basement', 'sqft_lot'], axis=1)
...
def feature_engineering(df):
    ...
    zipcode_median_price = df[['price', 'zipcode', 'living_size', 'lot_size']].groupby(
        ['zipcode', 'living_size', 'lot_size']).median().reset_index()
    zipcode_median_price.columns = [
        'zipcode', 'living_size', 'lot_size', 'region_median_price']

    df = pd.merge(df, zipcode_median_price, on=[
                  'zipcode', 'living_size', 'lot_size'], how='inner')

    # create the median price per m2 per region, living_size and lot_size
    zipcode_median_price_m2 = df[['price_m2', 'zipcode', 'living_size', 'lot_size']].groupby(
        ['zipcode', 'living_size', 'lot_size']).median().reset_index()
    zipcode_median_price_m2.columns = [
        'zipcode', 'living_size', 'lot_size', 'region_median_price_m2']

    df = pd.merge(df, zipcode_median_price_m2, on=[
                  'zipcode', 'living_size', 'lot_size'], how='inner')
    ...
    df['perc_value_below_median_price'] = (
        1 - (df['price']/df['region_median_price']))*100
    ...
    sorted_df = df.sort_values(by=['perc_value_below_median_price',
                               'condition', 'profit_est'], ascending=[False, True, False])
    ...
    sorted_df.loc[(sorted_df['date'].dt.month == 3) & (
        sorted_df['date'].dt.day > 19), 'season'] = 'spring'
    sorted_df.loc[(sorted_df['date'].dt.month == 6) & (
        sorted_df['date'].dt.day > 20), 'season'] = 'summer'
    sorted_df.loc[(sorted_df['date'].dt.month == 9) & (
        sorted_df['date'].dt.day > 21), 'season'] = 'autumn'
    sorted_df.loc[(sorted_df['date'].dt.month == 12) & (
        sorted_df['date'].dt.day > 20), 'season'] = 'winter'

    # Calculate the median price per season per region, living_size and lot_size
    groups_zip_sizes_season = sorted_df[['price', 'zipcode', 'living_size', 'lot_size', 'season']].groupby(
        ['zipcode', 'living_size', 'lot_size', 'season'])
    median_price_per_season = groups_zip_sizes_season.median().reset_index()
    median_price_per_season.columns = [
        'zipcode', 'living_size', 'lot_size', 'season', 'median_price']
    unique_indices = median_price_per_season[[
        'zipcode', 'living_size', 'lot_size']].drop_duplicates()
    best_season_df = pd.DataFrame(columns=[
    ...
    for row in unique_indices.iterrows():
        ...
        rows_indeces = (median_price_per_season['zipcode'] == row[1]['zipcode']) & \
                       ...
                       (median_price_per_season['lot_size']
                        == row[1]['lot_size'])

        unique_df = median_price_per_season.loc[rows_indeces]
        max_median_price = unique_df['median_price'].max()

        row_append = unique_df.loc[unique_df['median_price']
                                   == max_median_price]
        row_append.columns = ['zipcode', 'living_size',
                              'lot_size', 'best_season', 'best_season_median_price']

        best_season_df = best_season_df.append(row_append, ignore_index=True)
    houses_best_selling_price = pd.merge(sorted_df, best_season_df, how='left', on=[
    ...
    houses_best_selling_price['best_season_profit_est'] = houses_best_selling_price[
        'best_season_selling_price'] - houses_best_selling_price['price']
